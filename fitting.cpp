...
double get_phase(const int mode, double *per_pars, const double jd) {
	// given an array of parameters per_pars, compute the phase corresponding to a given jd
	double out_phase;
	switch (mode) {
		case 0:
...
void get_template(double *a, const int nf, const double phase, double &del_rho, double &del_tau, double &del_drho) {
	// calculate templates for given phase
	del_rho = 0;
	del_tau = 0;
	del_drho = 0;
	...
	for (int i=0;i<nf;i++) {
		// use recurrent relations to calculate cos and sin (much faster)
		ctt = ct*dct - st*dst;
		stt = st*dct + ct*dst;
...
double dphase_dt(const int mode, double *per_pars) {
	...
	switch(mode) {
		case 0:
			...
			break;
		case 1:
			// some other period law -- probably will be very similar to 1/P
			break;
		default:
			out = 1.0/per_pars[0];
			break;
...
int calculate_residuals(int n_tot, int n_coef, double *a, double *dy, double **derivs, void *vars) {
	...
	// if working with a large number of datapoints this is the place to make use of OpenMP - untested!!!!
	...
	for (int i=0;i<n_tot;i++) {
		...
		// make use of only data taken after JD 2 440 000
